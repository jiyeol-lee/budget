...
// AnalyzeDocument sends an image or PDF with a prompt to the AI and returns the response
func (c *Client) AnalyzeDocument(
	...
	case "image/jpeg", "image/png", "image/gif", "image/webp":
		contentBlock = anthropic.NewImageBlockBase64(mimeType, base64Data)
	...
	default:
		return "", fmt.Errorf("%w: unsupported mime type: %s", ErrInvalidImage, mimeType)
	}
...
// AnalyzeImage is deprecated, use AnalyzeDocument instead
func (c *Client) AnalyzeImage(
	ctx context.Context,
	base64Image, mimeType, prompt string,
) (string, error) {
	return c.AnalyzeDocument(ctx, base64Image, mimeType, prompt)
}
...
// Deprecated: Use ReceiptProcessingPrompt and ProcessReceiptImage instead
...
// ProcessReceipt is deprecated - use AnalyzeImage instead
func (c *Client) ProcessReceipt(imageData []byte) (*ReceiptData, error) {
	return nil, errors.New("use AnalyzeImage instead")
}

// ProcessReceiptImage performs OCR extraction and categorization in a single AI request
func (c *Client) ProcessReceiptImage(
	ctx context.Context,
	base64Image, mimeType string,
	budgets []string,
) (*ReceiptProcessingResult, error) {
	prompt := ReceiptProcessingPrompt(budgets)

	responseText, err := c.AnalyzeImage(ctx, base64Image, mimeType, prompt)
	if err != nil {
...
// Deprecated: Use ProcessReceiptImage instead
// ExtractReceiptItems performs OCR extraction on a receipt image (Step 1)
