...
const (
	...
	// FormFileKey is the key for the image file in the multipart form
	FormFileKey = "image"
)
...
type ReceiptHandler struct {
	aiClient            *ai.Client
	imageProcessor      *ai.ImageProcessor
...
func NewReceiptHandler(
	...
	return &ReceiptHandler{
		aiClient:            aiClient,
		imageProcessor:      ai.NewImageProcessor(),
...
// Accepts multipart form data with an image file and returns extracted receipt items
func (h *ReceiptHandler) Process(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.respondReceiptError(
			w,
			http.StatusBadRequest,
			"No image file provided. Use form field 'image'",
			models.ErrCodeInvalidImage,
		)
		return
	}
	...
	// Process the image
	processedImage, err := h.imageProcessor.ReadAndProcessReader(file)
	if err != nil {
		...
		if errors.Is(err, ai.ErrDecodeImage) {
			h.respondReceiptError(
				w,
				http.StatusBadRequest,
				"Could not decode image. File may be corrupted",
				models.ErrCodeInvalidImage,
			)
			return
		}
	...
	fmt.Printf("[Receipt] Image processed: mimeType=%s, dataLength=%d\n", processedImage.MimeType, len(processedImage.Base64Data))
