<script lang="ts">
	...
	let _expenses = $derived(expectedExpensesStore.expenses);
	...
	let _estimatedMonthlyTotal = $derived(
		safeNumber(expectedExpensesStore.getEstimatedMonthlyTotal())
	);
...
<div class="space-y-4 sm:space-y-6">
	...
	<div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
		...
		<div class="flex space-x-2">
			...
			<select
				value={selectedYear}
				onchange={(e) => handleDateChange(selectedMonth, parseInt(e.currentTarget.value))}
				class="border-input-border bg-input-bg text-text-primary focus:border-input-focus focus:ring-input-focus block w-full rounded-md shadow-sm sm:text-sm"
			>
