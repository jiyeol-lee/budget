...
<div class="space-y-4 sm:space-y-6">
	...
	{#if currentStep === 1}
		<div class="bg-surface rounded-lg p-6 shadow">
			...
			<ImageUploader onImageSelected={handleImageSelected} maxSizeMB={10} accept="application/pdf" />
		</div>
	{/if}
		...
		<div class="bg-surface rounded-lg p-6 shadow">
			...
			<div class="mb-6">
				...
				{#if receiptStore.selectedImage}
					<div class="flex h-64 w-full flex-col items-center justify-center bg-gray-100 dark:bg-gray-800">
...
{#if showConfirmModal}
	<div
		...
		onkeydown={(e) => e.key === 'Escape' && handleCancelModal()}
		role="dialog"
		aria-modal="true"
	...
	>
		<div
			class="bg-surface max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg p-4 shadow-xl sm:p-6"
		>
			<h3 id="modal-title" class="text-text-primary text-lg font-semibold">
				{m.receipt_confirm_add()}
			</h3>
