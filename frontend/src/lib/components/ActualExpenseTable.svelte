					...
					{#snippet tableFooter()}
						{#if meta.taxItem}
							<tr class="border-t border-amber-800 bg-amber-950/30">
								...
								<td class="whitespace-nowrap px-4 py-4 sm:px-6">
									<span
										class="inline-flex items-center rounded-full bg-amber-900 px-2.5 py-0.5 text-xs font-medium uppercase text-amber-200"
									>
										{m.expenses_type_tax()}
									</span>
								</td>
			...
			>
				<div class="mb-3 flex items-start justify-between">
					<div class="min-w-0 flex-1">
						<div class="flex items-center space-x-2">
							{#if expense.receipt_number}
								<span class="text-text-secondary text-xs font-medium">#{expense.receipt_number}</span
								>
							{/if}
