<script lang="ts">
	...
	// Derived values
	let isEditMode = $derived(expense !== null);
	let formTitle = $derived(isEditMode ? m.expense_edit_title() : m.expense_add_title());
...
<div class="bg-surface max-h-[90vh] overflow-y-auto rounded-lg p-6 shadow-lg">
	<h2 class="text-text-primary mb-6 text-lg font-semibold">{formTitle}</h2>
		...
		<div>
			...
			{#if touched.item_name && errors.item_name}
				<p class="text-danger mt-1 flex items-center text-sm">
					<AlertCircleIcon class="mr-1 h-4 w-4 flex-shrink-0" />
					{errors.item_name}
				</p>
			{/if}
		</div>
